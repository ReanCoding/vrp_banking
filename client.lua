...
if enableBankingGui then
  Citizen.CreateThread(function()
    while true do
	  ...
	  TriggerEvent('chatMessage', "", {255, 255, 255}, "Tryk ^5E ^0på for at tilgå banken");
        end
        atBank = true
        if IsControlJustPressed(1, 51)  then -- IF INPUT_PICKUP Is pressed
          if (IsInVehicle()) then
            TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Du kan ikke bruge banken i et køretøj!");
          else
            if bankOpen then
              closeGui()
              bankOpen = false
...
AddEventHandler('bank:deposit', function(amount)
  if(IsNearBank() == true or depositAtATM == true and IsNearATM() == true or depositAnywhere == true ) then
    if (IsInVehicle()) then
      ...
      TriggerEvent('chatMessage', "BANK", {0, 200, 0}, "^0Du har indsat ^5DKK "..amount);
    end
  else
...
AddEventHandler('bank:givecash', function(toPlayer, amount)
  if(IsNearPlayer(toPlayer) == true or giveCashAnywhere == true) then
    ...
    if (playing ~= false) then
      ...
      TriggerEvent('chatMessage', "", {0, 255, 0}, "^1Denne spiller er ikke online!");
    end
  else
    TriggerEvent('chatMessage', "", {0, 255, 0}, "^1Du er ikke nær denne spiller!");
  end
end)
