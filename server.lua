...
AddEventHandler("chatMessage", function(s, n, m)
  ...
  if command[1] == "/bank" then
    TriggerClientEvent("chatMessage", s, "BANK", {0, 200, 0}, "Din nuværende balance er: ^3DKK" .. bankbalance)
    TriggerClientEvent("banking:updateBalance", source, bankbalance)
    CancelEvent()
  end
  ...
  if command[1] == "/transfer" then
    if(command[2] ~= nil and tonumber(command[3]) > 0) then
      ...
      TriggerClientEvent('chatMessage', s, "BANK", {0, 200, 0}, "^1brug formatet /transfer [id] [mængde]^0")
    end
    CancelEvent()
  end
end)
...
AddEventHandler('bank:deposit', function(amount)
  if not amount then return end
    ...
    if(string.len(rounded) >= 9) then
      vRPclient.notify(user_id,{"~r~For højt tal."})
    else
      local bankbalance = vRP.getBankMoney({user_id})
      if(rounded <= wallet) then
        ...
        vRPclient.notify(user_id,{"~r~Ikke nok kontanter!"})
      end
    end
end)
  ...
  if not amount then return end
    ...
    if(string.len(rounded) >= 9) then
      ...
      if(tonumber(rounded) <= tonumber(bankbalance)) then
        ...
        vRPclient.notify(user_id, {"~r~Ikke nok penge på konto!"})
      end
    end
end)
...
AddEventHandler('bank:transfer', function(fromPlayer, toPlayer, amount)
  ...
  if tonumber(fPlayer) == tonumber(toPlayer) then
    TriggerClientEvent('chatMessage', fPlayer, "", {0, 255, 0}, "^1Du kan ikke overføre til dig selv")
    CancelEvent()
  else
    local rounded = round(tonumber(amount), 0)
    if(string.len(rounded) >= 9) then
      TriggerClientEvent('chatMessage', fPlayer, "", {0, 255, 0}, "^1For højt tal!")
      CancelEvent()
    else
      local bankbalance = vRP.getBankMoney({user_id})
      if(tonumber(rounded) <= tonumber(bankbalance)) then
        ...
        TriggerClientEvent('chatMessage', fPlayer, "", {0, 255, 0}, "^0Overført: ^1DKK"..math.floor(rounded))
        TriggerClientEvent('chatMessage', fPlayer, "", {0, 255, 0}, "^0Ny balance: ^4DKK" .. math.floor((bankbalance - rounded)))
        ...
        TriggerClientEvent('chatMessage', toPlayer, "", {0, 255, 0}, "^0Modtaget: ^2DKK"..math.floor(rounded))
        TriggerClientEvent('chatMessage', toPlayer, "", {0, 255, 0}, "^0Ny balance: ^4DKK" .. math.floor((bankbalance2 + rounded)))
        CancelEvent()
      else
        TriggerClientEvent('chatMessage', fPlayer, "", {0, 255, 0}, "^1Ikke nok penge på konto!")
      end
    end    
  end
end)
